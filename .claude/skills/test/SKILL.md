...
1. bridge port 확인:
   ```bash
   cat run/.agent/bridge-server.json
   ```
   파일이 없거나 연결 실패 시: "Minecraft 서버를 먼저 실행해주세요" 안내 후 중단.

2. 서버 연결 확인:
   ```bash
   curl -s http://localhost:{port}/session/info
   ```

3. 시나리오 로드: `docs/quality/scenarios/` 에서 해당 ID를 찾아 읽는다.
   - S-G → `gathering.md`
   - S-F → `farming.md`
   - S-X → `composite.md`
